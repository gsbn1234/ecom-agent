...
★★ 为什么需要这个文件（这是一次真实的排查教训，不是假想的）：

   CI 上 needs_browser 全红，注解只给到：

       local_browser_watchdog.py:428 in _wait_for_cdp_url
         raise RuntimeError(
       RuntimeError:

   ★ 先去读了库源码，两个事实决定了"继续翻 traceback"是死路：
     1. 第 428 行的 RuntimeError 消息是【硬编码】的，全文是
        "Browser process (PID n) exited before CDP became available ... e.g. --no-sandbox
         may be needed in Docker/headless environments, or a virtual display such as Xvfb"。
        把 --tb=short 改成 --tb=long 只是把这句我已经读过源码的话打全 —— 零新信息。
     2. 第 146-151 行用 stderr=asyncio.subprocess.PIPE 起了 Chrome，
        而全文件【没有任何一行读这个管道】（第 377 行那个 stderr 属于
        _install_browser_with_playwright，另一个函数）。
        → Chrome 临死前喊的那句话，被接了管道却没人听，直接丢了。

   所以真正缺的证据不在 Python 的 traceback 里，在【Chrome 自己的 stderr】里。
   这个探针就是去把它捞回来：同样的二进制、同样的 get_args()、同样的 headless 设置，
   唯一区别是【由我们自己读管道】。

★ 为什么用库自己的 get_args() 而不是自己拼一份参数：
  自己拼的参数即使能让 Chrome 起来，证明的也只是"我拼的那份能起来"。
  要回答的问题是"browser-use 起不来，为什么"，那就必须用它真正会用的那份参数 ——
  包括那三个 --load-extension（CI 上路径不存在，本身就是一个嫌疑人）。
  唯一额外加的是 --remote-debugging-port，因为库也是这么加的（第 112-117 行）。

★ 为什么失败也 exit 0：
   它是探针，职责是【说明】，不是【判决】。红了由下游的 pytest 去红。
   探针自己变红只会制造第二个需要解释的红色，而"需要解释的红色"正是
   这次排查里最费时间的东西。
"""
...
# 这里给 15s：探针要的是"能不能起来"，一个健康的 headless Chrome
# 通常 1-3s 就绪；等满 30s 只会让探针本身变成 CI 里最慢的一步。
CDP_TIMEOUT_S = 15.0
...
def main() -> int:
    ...
    if not browser_path:
        _ann(
            "warning",
            "browser-use 找不到 Chrome —— 下游浏览器测试的红是【环境问题】，不是代码问题",
        )
        return 0
    ...
    session = BrowserSession(headless=headless, **session_kw)
    ...
    if stderr:
        print("-" * 70)
        print("Chrome 的 stderr（库接了管道但从不读它，这里是唯一能看到的地方）:")
        print("-" * 70)
        print(stderr)
        print("-" * 70)
    else:
        print("Chrome 的 stderr: （空）")

    # ── 4. 结论 ─────────────────────────────────────────
    if verdict == "cdp_ok":
        _ann("notice", "探针：按库的参数能起 Chromium 且 CDP 就绪 → 环境没问题，去查我们自己代码")
    elif verdict == "exited":
        tail = " | ".join(stderr.splitlines()[-4:]) if stderr else "(stderr 为空)"
        _ann("error", f"探针：Chrome 启动即退出（退出码 {code}）。stderr 末尾：{tail}")
    else:
        tail = " | ".join(stderr.splitlines()[-4:]) if stderr else "(stderr 为空)"
        _ann("error", f"探针：{CDP_TIMEOUT_S:.0f}s 内 CDP 没就绪，但进程还活着。stderr 末尾：{tail}")
