...
# 补充（Phase 6 收尾）：让产物自己说清「零行」是哪一种

## 事实 1 ★★★：空数据的零行 和 登录态失效的零行，在**所有**产物里长得一模一样

真站点首跑跑完（run `20260917T120507+0000-02eff0`）：

```
status=completed   parse_status=empty   rows_collected=0   退出码 0
report.html 齐全   两张真实后台截图      zero errors
```

它其实是**第二种**情况 —— 那个账号是刚入驻的新店，后台七个标签全是 0。
但产物里没有任何一个字段能说出这件事。当时能分辨的唯一办法，是**读 LLM 写的那句
中文 note**，或者人肉去点开截图看。也就是说：

> 结论靠的是**模型的措辞**，不是机制。换个措辞、或者换个不爱写 note 的模型，
> 就又回到"看不出为什么"。而那个原因的处置方向完全相反 ——
> 一个是"店里没数据，正常"，另一个是"cookie 过期了，去重新扫码"。

**为什么它比看起来更值得修**：那次是**空店帮我们暴露了它**。
店里有两百件商品时，某次 cookie 过期会给出**完全一样**的产物，
而且更难起疑（"上次还好好的"）。所以这个字段的价值与店里有货没货无关。

**为什么"记录配置的 profile"是没用的**：cookie 过期时，配置里照样老老实实写着
那个 profile 路径，而 run 照样停在登录页。它能回答"我们配了哪个 profile"，
回答不了"这次到底登进去没有"。有用的只有**观察到的**那一页。

## 处置：三条通道 + 一个三值判定

| 落点 | 位置 | 谁看 |
|---|---|---|
| `run.json` | `RunRecord.login_state` / `_reason` / `_url` | 回放、程序 |
| `report.html` | 「可信度」的**第一行** | 打开报告的人 |
| `runs` 表 + `main.py runs` | `login_state` 等三列 + 列表新列 | "最近几次零行是怎么回事" |

判定逻辑从 `devtools/login_pdd.py` **移进库**（`ecom_agent/runtime/loginstate.py`），
两个调用点共用同一份 —— 否则会出现"标记说就绪、run 说落在登录页"这种
自己跟自己矛盾的产物，而那个矛盾的排查成本远高于它挡住的问题。

四个取值，**四个都得是分开的**：

```
logged_in   页面上出现了登录后才有的导航（或 URL 不在登录路径上）
login_page  URL 落在登录路径上（优先于页面文字），或页面出现登录字样
unknown     探过了，但两边都不像
""（NOT_PROBED）  压根没探 —— 这个任务不需要登录态
```

★★ `""` 与 `unknown` **必须分开**：前者是"不需要探"（一切正常），
后者是"探了但看不清"（要人看一眼）。两种沉默的处置**相反**，
压成一个值，报告就没法说清它到底是哪种沉默 —— 而"说不清"正是这一行存在的理由。

★ `probe()` **绝不允许把 run 弄挂**：会话读不到（窗口关了 / CDP 断了）时返回
`unknown` 并把异常写进依据，而不是抛出去。否则"加了一个诊断字段"的净效果是
**多了一种 run 失败的方式**，那比不诊断更糟。

## 三个对照实验（都真跑过，不是推演）

1. **摘掉 `_build_record` 里那三行** → `assert '' == 'login_page'` 红。
   证明"探了但没落盘"会被抓住（这正是本项目反复出现的那个家族）。
2. **摘掉 `_run_once` 里那段探测** → 4 条红（"压根没探"/"需要登录态的任务却没探"/
   "探到的 URL 要原样记下来"/`'' == 'unknown'`）。
3. **摘掉列表那条 SQL 里的 `login_state`** → 红的形态很说明问题：
   那一行照常打出来，只是登录态列变成 `-` —— 也就是"库里有、列表里没有"
   这扇门**不报错、不告警，只是少了一列**。

另外 `tests/test_report.py` 的 6 条，摘掉 `_login_state_row` 之后**全红**。

## 真库上的迁移（不是合成库）

`runs/ecom_agent.db` 是审计资产（Phase 3/4/6 三次真 run 都在里面），
而它是**加这三列之前**建的 —— 正好是一次真的迁移：

```
INFO  给已存在的 runs 表补列：login_state
INFO  给已存在的 runs 表补列：login_state_reason
INFO  给已存在的 runs 表补列：login_state_url
（3 行老数据全在，登录态列显示 '-'）
```

★ 老行显示 `-` 是**诚实**的：那时确实没探过。给它填一个"已登录"才是撒谎，
而且是最坏的那种 —— 它会把一次真出事的历史记录洗成正常的。

## 守卫清单

| 守卫 | 挡住什么 |
|---|---|
| `tests/test_loginstate.py`（9 条） | 三值判定本身、`""` ≠ `unknown`、`probe` 不抛 |
...
# S8：机会商品页 —— 第一种"形态差异"，以及它撞出来的一个表语义问题（2026-09-17）

## 探的是什么，结论是什么

问题：`extract_table` 在真站点上采到 0 行，而我们**分不清**"这一页没数据"和
"这一页的数据不是表格"。所以 S8 只做一件事：**把那个 0 行变成一次测量。**

真读出来的（不是推演）：机会商品页真实存在且有数据 —— **35 张商品卡**，
每张是一个 `<span>`，卡内文本**正好四行**：

```
['<商品标题>', '¥15.1', '热度 82', '发布同款']
```

⚠️ 标题位置换成了占位符：**真实商品标题不进仓库**（README 已声明演示数据全部来自
本地 mock，抄一个真标题进来就和那句话打架了）。**形态是实测的** —— 四行、
第 2 行以 `¥` 开头、第 3 行是 `热度 N` 的写法、第 4 行是那个按钮；
`¥15.1` / `热度 82` 是这一张卡上的真实数值（不具识别性，留着它们是为了让
字段判据的正则**有真实形态可对**）。

而整页 `<table>` 数 = **0**。所以那一页读 0 行不是 bug，是 `extract_table`
只认表格。

两个刻意的做法，都是为了**让失败形态可区分**：

| 做法 | 挡的是什么 |
|---|---|
| 入口 URL 从**后台首页 DOM 上发现**，不硬编码 | 让"找不到入口"和"页面是空的"不长得一样（硬编码 URL 的话两者都表现为"导航到一个空页"） |
| 证据落盘：`discover.json` / `census.json` / 两张截图 / 每组的**未折叠** `lines` | 目视核对得到（我第一次漏了 `lines`，于是"结构判据命中"这件事只能靠我自己的话，只能重跑一次） |

顺带验到的两条：Layer 0 白名单放行 ✓；**登录态探针第一次在真后端上报
`logged_in`**（此前的 `logged_in` 都来自本地 mock）。

## 事实 1 ★★★：探测器的第一版把"最大的重复块"判成了 30 个行内埋点 `<script>`

**机制**（不是巧合）：`document.querySelectorAll('*')` 遍历所有元素，而 `<head>`
下那 30 个 `<script>` 是**同标签同 class 的兄弟** —— 按"同父同形兄弟"这条判据，
它们**真的**构成一个 30 个成员的组，比 35 张卡还……不，比任何一组都大。

**它为什么危险**：探测器**忠实地回答了一个问错了对象的问题**。判据没错，
对象错了。而更糟的是我给它写的 FAIL 文案是"机会商品页**没有采到任何一行**"——
页面明明有 35 张卡。**这是本项目反复吃亏的那个家族**：产物里一句话说得像
"页面是空的"，而真相是"我们的工具有问题"。

**处置**：① 不渲染标签进黑名单（`SCRIPT/STYLE/LINK/META/NOSCRIPT/TEMPLATE/HEAD`）；
② 结论改**三路**：采到了行 / 页面不空但没有 `<table>`（我们的读取器读不了）/
页面确实是空的；③ 证据落盘（见上表）。

## 事实 2 ★★：判据是**结构**，不是 class 名；字段判据在 YAML 里，而且是只读的

"同一父节点下 ≥N 个同标签同 class 的兄弟 = 一个列表" —— 它对 class 名**完全不敏感**，
这正是不用 class 名的理由：真实站点的 class 是构建产物 hash，随发版变，照抄它等于
埋一个必然过期的断言（与"mock 站点不复制真实站点 class 名"同一条理由）。
"列表"在 DOM 里的定义本来就是**重复**，而这条判据认的就是重复本身。

字段判据（哪一行算价格）**写在任务定义的 `card_fields` 里**（`{name, pattern}` 正则，
逐行匹配、第一个命中、有捕获组取第 1 组），走**闭包**进 action —— 所以模型能决定的
只有"读第几组、读多少"，**不能**决定"哪一行算价格"。

## 事实 3 ★★★：`products` 表记的是**这个店铺自己的商品** —— 于是第二个页面形态**不能**塞进去

写模板时才看清的（提方案时没看见这个岔路口）：机会商品页的行只有
**标题 / 价格 / 热度**三样，没有商品ID、没有库存、没有状态 —— **页面本来就没有**。

表层问题是形状：`ProductRow` 的 `goods_id/stock/status` 是必填，`products` 表那三列是
`NOT NULL`。硬套的唯一后果是**模型必须编造商品ID和库存**，正是本项目一路拒绝的事。

**但根子是语义**：这张表是 `goods_id/title/price/stock/status` + `goods_id_not_numeric`、
"库存 10 万件以上多半是把销量填进了库存"这类 sanity 判定 —— 它记的是**这个店铺自己的
商品**。而机会商品页是**平台推荐给这个店铺的商品**。塞进去不只是列对不上，
而是让"**这个店有多少商品**"这类问题从此答错。

同一条理由否掉了"给表加 `row_kind + extra_json`"那条路：那是把表的语义偷偷改宽，
比缺一个模板糟得多。

**代价对比（这是决定本身）**：要让"某个任务的行不进 products 表"这件事**被看见**，
它得穿过 8 个通道（run.json / 报告 / `runs` 表 / CLI / SSE 实时载荷 / 回放载荷 /
`/api/runs` / 三个前端渲染器）—— 也就是本库最贵的那一类改动（见本文件里那个
"同一类缺陷一晚上出现四次"）。而它换来的是计划里明确写着"**不是必要条件**"的加分项。

**结论**：`extract_cards` **动作交付**（它才是技术产出），
`tasks/pdd_chance_goods.yaml` **推迟**（它只是这个动作的声明方式）。
"products 表 = 店铺自己的商品"这条定位以前从没被写下来过，它是 Phase 7 的 ADR 材料。

## 三种失败形态，各自指向不同的处置

| 形态 | 结果 | 为什么这样设计 |
|---|---|---|
| 页面上没有够大的重复块 | error + **找到的组清单**（`[0] 6 个 <DIV> 例：'不限'`） | 不含清单，error 只能写"没找到列表"，而人在真站点上没法据此改 `group_index` |
| `group_index` 指到不存在的组 | error + 同一份清单 | "抓错组"唯一可诊断的形态 |
| 某字段在所有卡上都没匹配 | **不报错**：空值 + `missing` 计数 | 判据写错是**人的错**，报错会推着 LLM 去"想办法"，而它的办法通常是改用库内置的 `extract` —— 价格被转录一遍，错误从**看得见的空**变成**看不见的错** |

## 守卫：三层，缺一层就是"绿得没有意义"

| 层 | 文件 | 验什么 |
|---|---|---|
| 离线 27 条 | `tests/test_extract_cards.py` | **拿到 payload 之后**的逻辑：字段判据、missing 计数、三种失败形态、闭包门禁、DSL 侧校验 |
| **真 DOM 4 条** | `tests/test_extract_cards_dom.py` | **脚本本身**：4 张卡读成 4 行、`innerText` 的分行契约、`group_index` 选到另一组时可诊断、`min_members` 真的进了页内脚本 |
| 证伪 | 手工，临时脚本 | 拆掉机制 → **对应的那条必须红**。⚠️ 脚本是**跑完即弃**的（落在 gitignore 的 `runs/` 下，不在仓库里），所以这里给的是可照做的**配方**而不是一条路径：改一处 → 跑 → 记下红的是哪几条 → 还原 → md5 复核 |

**真 DOM 那 4 条的关键设计**：测试页面里放了 **6 个 `<script>`，故意比 4 张商品卡还多** ——
没有 SKIP 过滤时"成员最多的组"就是它们。所以 `pickedTag == "DIV"` 这条断言
**只在过滤真的生效时成立**，它不是顺手加的。

**证伪记录**（拆掉 `SKIP` 里的 `'SCRIPT'`）：4 条**全红**，且红的原因正是声称的机制 ——

```
候选组数不是 2（是 3）—— inventory=[{'tag': 'SCRIPT', 'memberCount': 6, 'sample': '埋点一'},
  {'tag': 'DIV', 'memberCount': 4, 'sample': '测试商品甲 ¥12.30 热度 88 发布同款'}, {'tag': 'A', ...}]
选中的组不是商品卡（是 <SCRIPT>）
```

它还顺带证明了一件事：`script.innerText` **非空**（sample 是"埋点一"）——
如果它是空的，那 6 个脚本会被 `!sample` 那条过滤顺手滤掉，
**我的对照组就会是空的**（绿得没有意义）。还原后与改动前逐字节一致
（当时的 md5 是 `02f492495872bc75fd29f2df3717d898` —— ⚠️ 那**不是**本文件现在的 md5：
同一个文件随后为下面那条边界说明又改过一次。这里记的是"还原那一刻确实是原件"，
不是"文件的身份证"）。

## 顺手改正的一句"比事实更强的话"

我在 `extract_cards.py` 顶部写过"值的转录仍然**不经过 LLM**"。这句话在**这一步**
是真的（返回的 JSON 逐字是页面原文），但端到端看是假的：一次 run 的最终产物
仍然是模型写的 —— `done` → 结构化输出 → `products` 行，模型要从这份 JSON 抄进
`output_model`。区别在于那一手拿到的是**已分好字段、带 missing 计数**的文本，
而不是一整页原文（后者正是库内置 `extract` 的输入）。这与 `extract_table` 同形，
不是本动作引入的。已在该文件里补上边界说明，并写明：
**抄错能被看见（校验 + sanity 标记），但不能被阻止** —— 这两件事不要混着说。
