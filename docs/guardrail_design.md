...
| 形态 | 实测结果 | 性质 / 成因 |
|---|---|---|
| `Authorization: Bearer <token>` | ✅ **已盖住**（2026-09-17 修） | 曾**是缺陷**：规则要求 `[:=]` 在关键字**之后**，而规范头部里冒号在 `Bearer` **之前** → 该分支对它的规范形态永不命中。`token:` / `api_key=` 是好的，所以它**看起来在工作** |
| `Authorization: Basic <base64>`／`Token <key>` | ✅ **已盖住**（2026-09-18 修） | 曾**是缺陷**：与上一条同族，但**不是同一件事** —— `Basic` 的值是标准 base64，字母表比 `Bearer` 的 base64url 多 `+ / =`（实测：只认 bearer 字母表时一个字都不盖），所以字符类是**并集**；`Token` 是 DRF 的 40 位十六进制，且与既有的 `token:` 关键字规则同名，另有一条回归防"修一处、坏一处" |
| `Authorization: <scheme> authentication is required` | ⚠️ 值里的**英文虚词**被盖 | **已知代价，不是缺陷**：锚头名挡不住它 —— 这次**头名是对的**，坐错位置的是值。**不是本次引入**：`Bearer` 那条在 2026-09-17 就已经命中，这次只是同一形状从 1 个 scheme 扩到 3 个。有一条**以「已知代价」命名**的用例钉着（见下） |
| 文本里的订单号 | 盖不住 | **取舍，不是缺陷**：**故意不做**「长数字串一律盖掉」—— pdd 的 `goods_id` 就是长数字，笼统盖掉会把任务真正要采的数据一起毁掉。订单号没有跨平台通用形态，只能由 YAML 的 `observability.redact_extra` 给精确形态 |

**修第一条时被实测改掉的第一直觉，值得留在这里**（这是本节最有用的一行）：
本来打算写「关键字 + 空白 + 值」这条笼统规则，真跑了一下发现它会**误伤散文** ——
...
最终锚在**头名**上（`authorization: bearer <值>`）。
**代价明写**：不在头部、单说一句 `Bearer <值>` 的地方（比如 JSON 体里的字段）**仍不盖**。

★ 一条更一般的教训：脱敏有**两个**失败方向 —— **漏**（该盖的没盖）和**滥**
（不该盖的盖了，把审计价值抹掉）。两侧都要有钉子，只钉"漏"是不够的。

**2026-09-18 把同一族剩下的两个实例也收了**（`Basic` / DRF `Token`），补的过程里
有三个点值得留在这里：

1. **"同族"不等于"同一件事"。** `Basic` 的值是 `base64(user:password)`（RFC 7617），
   标准 base64 的字母表比 `Bearer` 用的 base64url 多 `+ / =`。实测：
   `Authorization: Basic dXNlcjpwYXNz+dmVy/abc=` 在只认 bearer 字母表的规则下
   **一个字都不盖**。⚠️ 更坏的是字符类收窄后的产物：它会**只盖到 `+` 之前**，
   在盘上留一截可直接使用的凭证 —— 而那**看起来像盖成功了**。所以专设一条用例
   逐字符复核 `+ / =` 都不许剩。
   **两种编码长得像、字母表不同**，这正是本条当初没被顺手修掉的原因。
2. **`Token` 与既有规则同名。** DRF 的 scheme 名和 `redact.py` 里那条
   `token:` / `token=` 关键字规则**撞名**，所以单加一条回归钉住老形态 ——
   防"修一处、坏一处"。
3. ★★ **一条"想消掉残差但实测失败"的记录**（本节最有用的一条）：
   残差是"头名之后坐着一个纯小写英文词"（`…: Basic authentication is required`）。
   想消掉它的思路看起来无懈可击：凭证是 base64/hex、**不透明**，英文虚词**全小写**，
   于是加一条前瞻要求"值里至少含一个非小写字符"。真跑一遍 —— **前瞻恒真、等于没加**。
   原因：这条规则带 `(?i)`，而 **`re.IGNORECASE` 会把 `[A-Z]` 变成 `[A-Za-z]`**，
   判别信号被规则自己的旗标中和掉了。要救得写 `(?-i:...)` 局部关旗标：
   为省一个英文虚词，换来一个"旗标可以局部反转"的隐性前提，**不值当**。
   → 残差**留着、并明写**，另有一条**以「已知代价」命名**的用例钉着它
   （命名成"已知代价"而不是"期望行为"，是为了让后人真去修的时候那条会红、
   然后被迫读一遍理由再决定）。**这就是「量一下再决定」和「想一下再决定」的差别。**

反面的设计（**不是失效，是刻意选的失败方式**）：1~2 字的字面值（比如店铺名"小店"）
**不静默跳过，直接抛 `ValueError`** —— 跳过意味着把日志洗成不可读，而那个后果是静默的。

守卫：`tests/test_sanity_and_pii.py`。其中的对照实验要能证明**两个方向都会红**：
把锚点拆成笼统形态 → "没盖过头"那条红；把新规则停掉 → "盖住了"那条红。

⚠️ 这条缺口**曾经**是用 **`strict xfail`** 钉的 —— 它的含义是"已知缺口的机器可读记录"，
**不是"这条已经测过了"**。这个机制已实测跑通一次：`redact.py` 修好后它变成 XPASS
并主动让整轮失败，提醒删标记，然后才转正成真守卫。
